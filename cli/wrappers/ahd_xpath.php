...
$parser = dirname(dirname(__DIR__)). "{$DS}classes{$DS}parsing{$DS}AdvancedHtmlDom.php";
if (file_exists( $parser ) && is_readable( $parser ))
{
	require $parser;
}
else {
	echo "Can't open AdvancedHtmlDom.php";
}
...
function parseLinks($html, $contentType = 'text/html', $css = false)
{
	$links = array();
	$dom = null;
