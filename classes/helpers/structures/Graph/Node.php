...
// +-----------------------------------------------------------------------------+
// | Copyright (c) 2003 Sergio Gon�alves Carvalho                                |
// +-----------------------------------------------------------------------------+
...
namespace helpers\structures\Graph;

use helpers\structures\Graph;
 ...
 *
 * @author		Sergio Carvalho <sergio.carvalho@portugalmail.com>
 * @copyright	(c) 2004 by Sergio Carvalho
 * @package Structures_Graph
 */
/* }}} */
class Node {
    ...
    * Node graph getter
    *
    * @return	Graph	Graph where node is stored
    * @access	public
    */
    function &getGraph() {
        return $this->_graph;
    ...
    */
     ...
     *
     * @param      string  Key
     * @param bool $nullIfNonexistent
     * @return mixed Metadata Data stored in node under given key
     * @throws \Exception
     * @internal param nullIfNonexistent $boolean .
    ...
    function &getMetadata($key, $nullIfNonexistent = false) {
        if (array_key_exists($key, $this->_metadata)) {
            ...
            if ($nullIfNonexistent) {
                ...
                throw new \Exception('Node::getMetadata: Requested key does not exist', STRUCTURES_GRAPH_ERROR_GENERIC);
            }
        }
    }
    /* }}} */
    ...
    */
    ...
    function connectTo(&$destinationNode) {
        ...
        if (!is_a($destinationNode, 'Structures_Graph_Node')) {
            throw new \Exception('Structures_Graph_Node::connectTo received an object that is not a Structures_Graph_Node', STRUCTURES_GRAPH_ERROR_GENERIC);
        }
        // Nodes must already be in graphs to be connected
        if ($this->_graph == null) {
            throw new \Exception('Structures_Graph_Node::connectTo Tried to connect a node that is not in a graph', STRUCTURES_GRAPH_ERROR_GENERIC);
        }
        if ($destinationNode->getGraph() == null) {
            throw new \Exception('Structures_Graph_Node::connectTo Tried to connect to a node that is not in a graph', STRUCTURES_GRAPH_ERROR_GENERIC);
        }
        // Connect here
